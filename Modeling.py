...
###
### Pretrained Model (InceptionV3) with Data Augmentation
###
...
start_time = time()
train_features, train_labels = extract_features_with_augmentation(train_dir, num_train, batch_size,
                                                times_augmentation) # 4121.0559158325195s
print(time() - start_time)


with open('./train_features_augmentation_40k_InceptionV3', 'wb') as fp:
    pickle.dump(train_features, fp, protocol=4)
with open('./train_labels_augmentation_40k_InceptionV3', 'wb') as fp:
    pickle.dump(train_labels, fp)
